...
import { Network, Brain, Mic, FileText, AlertTriangle, Stethoscope, MapPin, CheckCircle, User, Zap, Beaker } from 'lucide-react'
import { useState } from 'react'

const LANGGRAPH_NODES = [
  { id: 'user', label: 'User Input', icon: User, color: '#3B82F6', x: 50, y: 180 },
  { id: 'stt', label: 'Speech-to-Text', icon: Mic, color: '#8B5CF6', x: 200, y: 80 },
  { id: 'symptom', label: 'Symptom Analysis', icon: Beaker, color: '#06B6D4', x: 380, y: 50 },
  { id: 'risk', label: 'Risk Assessment', icon: Zap, color: '#FACC15', x: 560, y: 120 },
  { id: 'emergency', label: 'Emergency Detection', icon: AlertTriangle, color: '#EF4444', x: 560, y: 240 },
  { id: 'specialist', label: 'Specialist Rec.', icon: Stethoscope, color: '#22C55E', x: 740, y: 180 },
  { id: 'report', label: 'Health Report', icon: FileText, color: '#A855F7', x: 880, y: 180 },
]

const LANGGRAPH_EDGES = [
  { from: 'user', to: 'stt' }, { from: 'user', to: 'symptom' },
  { from: 'stt', to: 'symptom' }, { from: 'symptom', to: 'risk' },
  { from: 'risk', to: 'emergency' }, { from: 'risk', to: 'specialist' },
  { from: 'emergency', to: 'specialist' }, { from: 'specialist', to: 'report' },
]
      ...
      <div className="max-w-6xl mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.2 }}
            ...
            Four specialized AI agents work together through LangGraph's orchestration framework,
            each contributing unique capabilities to deliver your health assessment.
          </motion.p>
        </div>

        <motion.div initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
          className="glass rounded-3xl p-6 overflow-x-auto">
          <div className="relative" style={{ minWidth: 950, height: 380 }}>
